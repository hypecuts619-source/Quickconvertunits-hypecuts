...
import { ArrowRightLeft, Sun, Moon, Search, Settings, ChevronDown, Copy, Check, X, RefreshCw, Palette, Trash2, History, TrendingUp, Calculator, Grid, ArrowRight, Share2 } from 'lucide-react';
...
const POPULAR = [
  ...
  {label:'Celsius → Fahrenheit', cat:'temperature', fu:'celsius', tu:'fahrenheit'},
  {label:'Fahrenheit → Celsius', cat:'temperature', fu:'fahrenheit', tu:'celsius'},
  {label:'Meters → Feet', cat:'length', fu:'meter', tu:'foot'},
  {label:'Feet → Centimeters', cat:'length', fu:'foot', tu:'centimeter'},
  {label:'Inches → Centimeters', cat:'length', fu:'inch', tu:'centimeter'},
  {label:'Liters → Gallons', cat:'volume', fu:'liter', tu:'us_gallon'},
  {label:'Gigabytes → Megabytes', cat:'data', fu:'gigabyte', tu:'megabyte'},
];

const FORMULAS = [
  {title:'Celsius → Fahrenheit', code:'°F = (°C × 9/5) + 32', note:'Water boils at 100°C / 212°F'},
  {title:'Fahrenheit → Celsius', code:'°C = (°F − 32) × 5/9', note:'Water freezes at 0°C / 32°F'},
  {title:'Kilometers → Miles', code:'mi = km × 0.621371', note:'1 km ≈ 0.621 miles'},
  {title:'Miles → Kilometers', code:'km = mi × 1.60934', note:'1 mile ≈ 1.609 km'},
  {title:'Kilograms → Pounds', code:'lb = kg × 2.20462', note:'1 kg ≈ 2.205 pounds'},
  {title:'Meters → Feet', code:'ft = m × 3.28084', note:'1 meter ≈ 3.281 feet'},
  {title:'Hectares → Acres', code:'ac = ha × 2.47105', note:'1 hectare ≈ 2.471 acres'},
  {title:'Liters → US Gallons', code:'gal = L × 0.264172', note:'1 liter ≈ 0.264 gallons'},
];
...
const AdSlot = ({ widthClass, heightClass, className, text, mobileText }: { widthClass: string, heightClass: string, className?: string, text: string, mobileText?: string }) => {
  return (
    <div className={`flex flex-col items-center ${className || ''}`}>
      <span className="text-[10px] text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-1.5 font-medium">Advertisement</span>
      <div className={`flex items-center justify-center bg-neutral-100 dark:bg-[#1a1a1a] text-neutral-400 dark:text-neutral-600 text-xs font-medium border border-dashed border-neutral-200 dark:border-neutral-800 rounded-lg overflow-hidden ${widthClass} ${heightClass}`}>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-[#111111] border-t border-neutral-200 dark:border-neutral-800 z-[60] flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] text-center md:text-left">
      <p className="text-sm text-neutral-600 dark:text-neutral-400">
        ...
        By using our site, you consent to our <Link to="/privacy-policy" className="underline hover:text-primary-600 dark:hover:text-primary-400">Privacy Policy</Link>.
      </p>
      <button 
        onClick={() => {
...
function ConversionChart({ categoryId, valFrom, unitFrom, theme }: { categoryId: string, valFrom: string, unitFrom: string, theme: string }) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const formatted = val >= 1000 || val < 0.01 ? val.toExponential(4) : val.toPrecision(4);
      return (
        <div className="bg-white dark:bg-neutral-800 p-3 shadow-lg rounded-xl border border-neutral-200 dark:border-neutral-700 text-sm">
          <p className="font-semibold text-neutral-900 dark:text-neutral-100">{payload[0].payload.fullName}</p>
          <p className="text-primary-600 dark:text-primary-400 mt-1">{formatted} {label}</p>
        </div>
      );
    }
    return null;
...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem('theme-color') || 'blue');
  ...
  const [historyItems, setHistoryItems] = useState<HistoryItem[]>(() => {
    try { return JSON.parse(localStorage.getItem('qcu_hist') || '[]'); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    if (!isNaN(num)) {
      ...
      setValTo(Number.isInteger(res) ? res.toString() : parseFloat(res.toFixed(6)).toString());
    }
    ...
    if (valFrom && valTo && valFrom !== '0' && valTo !== '0') {
      const timer = setTimeout(() => {
        ...
        setHistoryItems(prev => {
          const filtered = prev.filter(h => !(h.fu === newEntry.fu && h.tu === newEntry.tu && h.cat === newEntry.cat));
  ...
  const handleValToChange = (v: string) => {
    ...
    if (!isNaN(num)) {
      ...
      setValFrom(Number.isInteger(res) ? res.toString() : parseFloat(res.toFixed(6)).toString());
    }
  };
  ...
  useEffect(() => {
    if (activeFromUnit && activeToUnit) {
      const titleStr = `${activeFromUnit.name} to ${activeToUnit.name} Converter`;
      document.title = `${titleStr} - QuickConvert`;
      ...
      metaDesc.setAttribute('content', `Fast and free ${activeCategory.name.toLowerCase()} calculator. Easily convert ${activeFromUnit.name} to ${activeToUnit.name} and explore our wide range of units.`);
        ...
        },
        {
          ...
          "mainEntity": [
            ...
            },
            {
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": activeFromUnit.description || `A ${activeFromUnit.name} is a unit of ${activeCategory.name.toLowerCase()}.`
              }
            }
          ]
        }
  ...
  }, [category, unitFrom, unitTo, activeFromUnit, activeToUnit, activeCategory, valFrom]);

  return (
    <div className={`min-h-screen text-neutral-900 dark:text-neutral-100 font-sans transition-colors duration-200 relative overflow-hidden`}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-12 flex flex-col lg:flex-row gap-8 pb-24 md:pb-12 relative z-10">
        ...
        <div className="flex-1 max-w-3xl mx-auto w-full">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-neutral-500 dark:text-neutral-400 text-lg font-light">
              Fast, accurate, and simple conversion from {activeFromUnit?.name} to {activeToUnit?.name}.
            </p>
          </div>
          ...
          {/* AD: Top Leaderboard / Mobile Banner (Moved from very top to prevent AdSense violation for pushing content below fold) */}
          <AdSlot 
            className="mb-10" 
            widthClass="w-full max-w-[728px]"
            heightClass="h-[50px] md:h-[90px]"
            text="Top Leaderboard Ad (728x90)" 
            mobileText="Top Banner Ad (320x50)" 
          />
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-8 items-center relative">
              ...
              <div className="flex-1 w-full bg-neutral-50/50 dark:bg-[#161616] rounded-3xl p-6 border border-neutral-100 dark:border-neutral-800 focus-within:border-primary-500 dark:focus-within:border-primary-400 focus-within:ring-4 focus-within:ring-primary-500/10 dark:focus-within:ring-primary-400/10 transition-all relative group">
                <label className="text-[10px] uppercase tracking-widest text-neutral-400 dark:text-neutral-500 font-semibold mb-3 block">From</label>
                <div className="relative">
                  <select 
                    value={unitFrom}
                  ...
                  >
                    {activeCategory.units.map(u => (
                      <option key={u.id} value={u.id} className="text-neutral-900 dark:text-neutral-100 bg-white dark:bg-neutral-800">
                        {u.name} ({u.symbol})
                      </option>
                    ))}
                  </select>
              ...
              {/* TO */}
              <div className="flex-1 w-full bg-neutral-50/50 dark:bg-[#161616] rounded-3xl p-6 border border-neutral-100 dark:border-neutral-800 focus-within:border-primary-500 dark:focus-within:border-primary-400 focus-within:ring-4 focus-within:ring-primary-500/10 dark:focus-within:ring-primary-400/10 transition-all relative group">
                <label className="text-[10px] uppercase tracking-widest text-neutral-400 dark:text-neutral-500 font-semibold mb-3 block">To</label>
                <div className="relative">
                  <select 
                    value={unitTo}
                ...
                <div className="relative flex items-center mt-3">
                  ...
                  {valTo && (
                    ...
                    >
                      {copied ? <Check className="w-6 h-6 text-green-500" /> : <Copy className="w-6 h-6 opacity-0 group-focus-within:opacity-100 hover:opacity-100 focus:opacity-100" />}
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="mt-6 flex flex-col items-center justify-center gap-2 text-sm text-neutral-400 dark:text-neutral-500">
              {valFrom && valTo && (
                 <p>1 {activeFromUnit?.symbol} = {
                   parseFloat((convert(1, unitFrom, unitTo, category)).toFixed(6))
                 } {activeToUnit?.symbol}</p>
              )}
              {category === 'currency' && (
                <button 
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
               <button onClick={handleCopy} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-neutral-100 dark:bg-[#1a1a1a] hover:bg-neutral-200 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-700 dark:text-neutral-300 transition-colors">
                 <Copy className="w-4 h-4" /> {copied ? 'Copied!' : 'Copy Result'}
               </button>
               <button onClick={handleShare} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-neutral-100 dark:bg-[#1a1a1a] hover:bg-neutral-200 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-700 dark:text-neutral-300 transition-colors">
                 <Share2 className="w-4 h-4" /> Share
               </button>
               <button onClick={() => setShowBulk(!showBulk)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${showBulk ? 'bg-primary-500/10 text-primary-600 dark:text-primary-400' : 'bg-neutral-100 dark:bg-[#1a1a1a] hover:bg-neutral-200 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300'}`}>
                 <Grid className="w-4 h-4" /> {showBulk ? 'Hide Bulk Convert' : 'Bulk Convert'}
               </button>
               <button onClick={handleClear} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-neutral-100 dark:bg-[#1a1a1a] hover:bg-neutral-200 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-700 dark:text-neutral-300 transition-colors">
                  ...
                  >
                     <div className="bg-neutral-50 dark:bg-[#161616] border border-neutral-100 dark:border-neutral-800 rounded-3xl p-6">
                        <div className="flex items-center gap-2 mb-4">
                           ...
                           <h3 className="font-semibold text-neutral-800 dark:text-neutral-200 tracking-tight">Bulk Conversion</h3>
                        </div>
                        <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-6">Converting <strong className="text-neutral-800 dark:text-neutral-200">{valFrom} {activeFromUnit?.name}</strong> to all {activeCategory.name.toLowerCase()} units.</p>
                        
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                           {activeCategory.units.map(u => {
                              if (u.id === unitFrom) return null;
                              return (
                                 <div key={u.id} className="bg-white dark:bg-[#1a1a1a] rounded-xl p-3 border border-neutral-200/60 dark:border-neutral-800/60 shadow-sm shadow-neutral-200/20 dark:shadow-none hover:border-primary-200 dark:hover:border-primary-900/40 transition-colors cursor-pointer group" onClick={() => setUnitTo(u.id)}>
                                    <div className="text-[10px] text-neutral-400 uppercase tracking-widest font-semibold mb-1 group-hover:text-primary-500 transition-colors">{u.name}</div>
                                    <div className="text-sm font-semibold font-mono text-neutral-800 dark:text-neutral-200 break-all">
                                      {parseFloat((convert(parseFloat(valFrom), unitFrom, u.id, category)).toFixed(6))} {u.symbol}
                                    </div>
                                 </div>
                              );
                           })}
          ...
          <ConversionChart categoryId={category} valFrom={valFrom} unitFrom={unitFrom} theme={theme} />
          ...
          <section className="mt-16 text-neutral-800 dark:text-neutral-200">
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-12">
               ...
               <div className="bg-white dark:bg-[#111111] rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none p-6">
                  <div className="flex items-center gap-2 mb-6">
                     ...
                     <h3 className="font-semibold text-lg tracking-tight">Popular</h3>
                  </div>
                  <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                     {POPULAR.map((p, i) => (
                        ...
                        >
                           <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{p.label}</span>
               ...
               <div className="bg-white dark:bg-[#111111] rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none p-6">
                  ...
                  {historyItems.length === 0 ? (
                     <div className="h-[200px] flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-neutral-500 dark:text-neutral-400 text-sm font-light">Your recent conversions<br/>will appear here.</p>
                     </div>
                  ) : (
                     <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                        {historyItems.map((h, i) => (
                           <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-[#1a1a1a] transition-colors group cursor-pointer" onClick={() => {
                                 handleCategoryChange(categories.find(c => c.name === h.cat)?.id || '');
                                 setTimeout(() => {
                                    // we do not store accurate full id's since h.fu is the symbol. 
                                    // But actually we need to find the unit by symbol.
                                    const catId = categories.find(c => c.name === h.cat)?.id;
                                    if(catId) {
                                       const actualFu = categories.find(c => c.id === catId)?.units.find(u => u.symbol === h.fu)?.id;
                                       const actualTu = categories.find(c => c.id === catId)?.units.find(u => u.symbol === h.tu)?.id;
                              ...
                              <div className="flex items-center gap-3">
                                 <span className="text-sm font-mono font-medium text-neutral-800 dark:text-neutral-200">{h.fv.length > 5 ? h.fv.substring(0, 5) + '..' : h.fv} {h.fu}</span>
                                 <ArrowRight className="w-3 h-3 text-neutral-400" />
                                 <span className="text-sm font-mono font-semibold text-primary-600 dark:text-primary-400">{h.tv.length > 5 ? h.tv.substring(0, 5) + '..' : h.tv} {h.tu}</span>
                              </div>
                              <div className="flex items-center gap-3">
            ...
            <div className="mb-12">
               <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="text-2xl font-semibold tracking-tight">Quick Reference Formulas</h3>
               </div>
               <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {FORMULAS.map((f, i) => (
                     <div key={i} className="p-5 rounded-2xl bg-white dark:bg-[#111111] border border-neutral-100 dark:border-neutral-800">
                        <h4 className="font-semibold text-sm mb-3 tracking-tight text-neutral-800 dark:text-neutral-200">{f.title}</h4>
                        <div className="bg-neutral-50 dark:bg-neutral-800/50 p-2.5 rounded-lg text-primary-600 dark:text-primary-400 font-mono text-sm font-medium mb-2 w-full truncate cursor-pointer hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors"
                           onClick={() => navigator.clipboard.writeText(f.code.split(' = ')[1] || f.code)}
                           title="Click to copy right side of equation"
                        >
                           {f.code}
                        </div>
                        <p className="text-xs text-neutral-500 dark:text-neutral-400">{f.note}</p>
                     </div>
                  ))}
               </div>
            </div>

            <h2 className="text-2xl font-semibold mb-4 tracking-tight">How to convert {activeFromUnit?.name} to {activeToUnit?.name}</h2>
            <p className="mb-6 leading-relaxed font-light text-neutral-600 dark:text-neutral-400">
              Our {activeCategory.name.toLowerCase()} converter makes it easy to convert between {activeFromUnit?.name.toLowerCase()} and {activeToUnit?.name.toLowerCase()}. 
              Simply enter the value you want to convert in the input fields above, and the calculator will automatically update with the correct result.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-6 my-10">
              {activeFromUnit?.description && (
                <div className="bg-white dark:bg-[#111] p-8 rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none">
                  <h3 className="font-semibold text-lg mb-3 tracking-tight">About {activeFromUnit.name}</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-sm leading-relaxed font-light">{activeFromUnit.description}</p>
                </div>
              )}
              {activeToUnit?.description && activeToUnit.id !== activeFromUnit?.id && (
                 <div className="bg-white dark:bg-[#111] p-8 rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none">
                  <h3 className="font-semibold text-lg mb-3 tracking-tight">About {activeToUnit.name}</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-sm leading-relaxed font-light">{activeToUnit.description}</p>
                </div>
              )}
            </div>

            <h3 className="text-xl font-semibold mb-3 tracking-tight">Understanding the Conversion Factors</h3>
            <p className="mb-8 leading-relaxed font-light text-neutral-600 dark:text-neutral-400">
              Whether you are a student, engineer, or just need a quick calculation on your phone, QuickConvert is designed to be fast and mobile-friendly. 
              It supports a wide range of units in the {activeCategory.name.toLowerCase()} category, including {activeCategory.units.slice(0, 3).map(u => u.name.toLowerCase()).join(', ')} and more.
            </p>

            <h3 className="text-xl font-semibold mb-4 tracking-tight">Frequently Asked Questions</h3>
            <div className="space-y-6">
              <div>
                <h4 className="font-medium text-lg mb-2 text-neutral-900 dark:text-white">How do I convert {activeFromUnit?.name} to {activeToUnit?.name}?</h4>
                <p className="font-light text-neutral-600 dark:text-neutral-400 text-sm">
                   To convert {activeFromUnit?.name} to {activeToUnit?.name}, you simply multiply or divide the given value based on the respective standard multiplier. Our tool handles this complex math behind the scenes for an instant and accurate result.
                </p>
              </div>
              <div>
                <h4 className="font-medium text-lg mb-2 text-neutral-900 dark:text-white">What is a {activeFromUnit?.name}?</h4>
                <p className="font-light text-neutral-600 dark:text-neutral-400 text-sm">
                  {activeFromUnit?.description || `A ${activeFromUnit?.name} is a standardized unit of ${activeCategory.name.toLowerCase()} used commonly across various applications.`}
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="mt-16 pt-8 border-t border-neutral-200 dark:border-neutral-800 text-neutral-600 dark:text-neutral-400 text-sm flex flex-col md:flex-row items-center justify-between gap-4">
            <p>&copy; {new Date().getFullYear()} QuickConvert. Built for fast, reliable unit conversions.</p>
            <div className="flex items-center gap-4">
              <Link to="/blog" className="hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Blog</Link>
        ...
        <aside className="hidden lg:block w-[300px] shrink-0">
          <div className="sticky top-24 space-y-6">
            {/* AD: Right Sidebar Sticky */}
            <AdSlot 
              widthClass="w-[300px]"
              heightClass="h-[600px]"
              text="Sticky Sidebar Ad (300x600)" 
            />
          </div>
        </aside>
